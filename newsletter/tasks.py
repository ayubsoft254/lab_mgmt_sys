...
def process_email_campaign(campaign_id):
    ...
    try:
        ...
        if campaign.recipient_type == 'csv_upload':
            # For CSV campaigns, delegate to CSV-specific task
            process_csv_campaign.delay(campaign_id)
        else:
            ...
            for delivery in deliveries:
                try:
                    ...
                    context = {
                        ...
                        'email': email,
                        'tracking_pixel': "{}/track/open/{}/".format(
...
def process_csv_campaign(campaign_id):
    """Process a CSV-based email campaign"""
    from .models import EmailCampaign, CsvRecipient
    
    try:
        campaign = EmailCampaign.objects.get(id=campaign_id)
        
        logger.info("Processing CSV campaign '{}' with {} recipients".format(
            campaign.name, campaign.total_recipients))
        ...
        logger.error("Error processing CSV campaign {}: {}".format(campaign_id, str(e)))
...
def process_csv_campaign_batch(campaign_id, batch_size=50):
    ...
    try:
        ...
        for offset in range(0, total_recipients, batch_size):
            ...
            # Create subtasks for each recipient in the batch
            batch_tasks = []
            for csv_recipient in batch_recipients:
                task = send_single_csv_email.delay(campaign.id, csv_recipient.id)
                batch_tasks.append(task)
            
            # Wait for batch to complete before processing next batch
            for task in batch_tasks:
                task.get()  # This waits for the task to complete
